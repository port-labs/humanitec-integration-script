...
def process_app_entities(apps_data: list[dict[str, Any]]):
    blueprint_id = "humanitec_application"
...
def process_environments_entities(environment_data: list[dict[str, Any]]):
    blueprint_id = "humanitec_environment"
...
def get_environments(app: dict[str, Any]):
    ...
    for environments_batch in get_paginated_resource(path=environments_path):
        logger.info(f"received environments batch with size {len(environments_batch)} from app: {app['name']}")
